...
class PostForm(FlaskForm):
    ...
    published_at = DateTimeLocalField(
        "Published at", validators=[Optional()], format="%Y-%m-%dT%H:%M"
    )
...
register_crud(
    ...
    list_cols=[
        ...
        (
            "Published",
            lambda p: p.published_at.strftime("%Y-%m-%d %H:%M") if p.published_at else "draft",
        ),
    ],
...
def posts_edit(id: int):
    ...
    return render_template(
        "admin/posts_edit.html", post=post, form=form, block_types=list(BLOCK_TYPES)
    )
